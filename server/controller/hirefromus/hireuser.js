...
*/
const hireregister = async (req, res) => {
  try {
    ...
    const existingEmail = await HireUser.findOne({ email: email });
    const existingPhone = await HireUser.findOne({ phone: phone });
...
const userdetails = async (req, res) => {
  try {
    ...
    const user = await HireUser.findOne({ _id: _id });
    if (!user) {
...
const getalluser = async (req, res) => {
  try {
    const user = await HireUser.find({});
    res.status(200).json({
      success: true,
